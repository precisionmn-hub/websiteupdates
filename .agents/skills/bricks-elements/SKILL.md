...
description: When the user wants to build a custom element for Bricks Builder — a PHP class with controls, control groups, and render logic. Also use when the user mentions "custom Bricks element," "register a Bricks element," "new element for Bricks," "add a control to a Bricks element," "Bricks element PHP class," "element.php," "set_controls," "set_control_groups," "Bricks\\Element," "custom widget for Bricks," or asks for a reusable element like a team card, pricing tier, feature box, or counter. Produce PHP that extends Bricks\\Element with proper controls, tag, category, and render() output.
metadata:
  version: 1.0.0
